#RTFormer is copied from https://github.com/PaddlePaddle/PaddleSeg/blob/release/2.7/paddleseg/models/rtformer.py
...
class RTFormer(nn.Layer):
    ...
    def __init__(self,
        ...
        if self.use_aux_head:
            self.seghead_extra = Lane_exist(cfg,cfg.num_classes-1,base_channels*2)
    ...
    def forward(self, x):
        ...
        if self.training and self.use_aux_head:
            ...
            output.update({'exist':x_out_extra})

        logit_list = [
            F.interpolate(
                logit,
                [self.cfg.img_height,  self.cfg.img_width],
                mode='bilinear',
                align_corners=False) for logit in logit_list
        ]
        output.update({'seg':logit_list[0]})
