...
class non_bottleneck_1d(nn.Layer):
    def __init__(self, chann, dropprob, dilated):
        ...
        self.conv3x1_1 = nn.Conv2D(
            chann, chann, (3, 1), stride=1, padding=(1, 0), bias_attr=True)

        self.conv1x3_1 = nn.Conv2D(
            chann, chann, (1, 3), stride=1, padding=(0, 1), bias_attr=True)

        self.bn1 = nn.BatchNorm2D(chann, epsilon=1e-03)
        ...
        self.bn2 = nn.BatchNorm2D(chann, epsilon=1e-03)
...
class UpsamplerBlock(nn.Layer):
    def __init__(self, ninput, noutput, up_width, up_height):
        ...
        self.conv = nn.Conv2DTranspose(
            ninput, noutput, 3, stride=2, padding=1, output_padding=1, bias_attr=True)

        self.bn = nn.BatchNorm2D(noutput, epsilon=1e-3)

        self.follows = nn.LayerList()
