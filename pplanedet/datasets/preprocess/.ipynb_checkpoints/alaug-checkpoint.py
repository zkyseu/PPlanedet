...
class Alaug(object):
    ...
    def build_transforms(self, transform):
        if transform['name'] == 'OneOf':
            transforms = transform['transforms']
            choices = []
            for t in transforms:
                parmas = {
                    key: value
                    for key, value in t.items() if key is not 'name'
                }
                choice = getattr(al, t['name'])(**parmas)
                choices.append(choice)
            return getattr(al, 'OneOf')(transforms=choices, p=transform['p'])

        parmas = {
            key: value
            for key, value in transform.items() if key is not 'name'
        }
        return getattr(al, transform['name'])(**parmas)
    ...
    def build(self):
        ...
        self.__augmentor._disable_check_args()
