...
def loss_computation(logits_list, labels_list, edges, losses):
...
def train(model,
    ...
    while iter < iters:
        for data in loader:
            ...
            if "exist_lane_num" in data.keys():
                exist_lane_num = data["exist_lane_num"]
                labels = [labels,exist_lane_num]
            else:
                logger.info("this model does not use exist lane loss")
            edges = None
            ...
            # print("image:",paddle.unique(images))
            ...
            if (iter % save_interval == 0 or iter == iters) and local_rank == 0:
                ...
                if val_dataset is not None:
                    ...
                    logger.info(
                        '[EVAL] The model with the best validation best_accuracy ({:.4f}) was saved at iter {}.'
                        .format(best_accuracy, best_model_iter))
